<template lang="pug">
  v-layout.tab-panel-content(xs12)
    v-flex.tab-content-block.secondary.pa-2.white--text(xs4) {{ group.name }}
    v-flex.tab-content-block.secondary.lighten-1.pa-2.white--text(xs4) {{ view.name }}
...
export default {
  ...
  computed: {
    ...
    group() {
      return this.getEntityItem(ENTITIES_TYPES.group, this.view.group_id);
    },
  },
};
</script>
