...
import { isAlarmPatternField, isEntityPatternField, isPbehaviorPatternField } from '@/helpers/search/advanced-search';
 ...
 * @param {Object} options.groups - Object mapping group keys to arrays of items.
 * @param {string} [options.aliasGroupKey=ADVANCED_SEARCH_GROUPS.alias] - Key of the alias group.
 * @param {Array|Function} options.aliases - Alias items (can be reactive).
 * @param {Function} options.prepareItem - Function to prepare regular items.
 * @param {boolean|Function} options.disabled - Flag indicating if items should be disabled (can be reactive).
 * @returns {Object} An object containing the computed `items` array.
 * @property {Array} items - A computed array of items with headers.
 */
export const useAdvancedSearchGroupedAttributes = ({
  ...
  const items = computed(() => {
    ...
    const result = Object.entries(groups).reduce((acc, [group, groupItems]) => {
      ...
      acc.push(
        { header, value: header },
        ...groupItems.map(item => prepareItem(item, unwrappedDisabled)),
      );
...
export const useAlarmAdvancedSearchAttributes = ({
  ...
  const entitiesAttributesMapWithPrefix = computed(() => (
    Object.entries(entityAttributesMap.value).reduce((acc, [key, value]) => {
      acc[`${ALARM_ADVANCED_SEARCH_PATTERNS_PREFIXES.entity}${key}`] = value;

      return acc;
    }, {})
  ));

  const pbehaviorsAttributesMapWithPrefix = computed(() => (
    Object.entries(pbehaviorAttributesMap.value).reduce((acc, [key, value]) => {
      acc[`${ALARM_ADVANCED_SEARCH_PBEHAVIOR_PATTERN_PREFIX}${key}`] = value;
