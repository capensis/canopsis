 ...
 */
export const formToScenarioAction = (form, author, timezone) => {
  ...
  // TODO: Should be removed after backend finish changes
 ...
 * @returns {Scenario}
 */
export const formToScenario = (form, author, timezone = moment.tz.guess()) => ({
  ...omit(form, ['delay', 'actions']),
  author,
  delay: form.delay && isNumber(form.delay.value)
    ? formToDuration(form.delay)
    : undefined,
  actions: form.actions.map(action => formToScenarioAction(action, author, timezone)),
});
