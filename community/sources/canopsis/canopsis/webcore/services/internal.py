...
VALID_USER_INTERFACE_PARAMS = [
    ...
    'allow_change_severity_to_info', 'max_matched_items'
]
...
def get_login_config(ws):
    ...
    for login_service in records:
        ...
        if login_service_name == 'casconfig':
            login_service['server'] = login_service.get('server', '').rstrip('/')
            login_service['service'] = login_service.get('service', '').rstrip('/')
...
def exports(ws):
    ...
    def update_internal_interface():
        ...
        if 'max_matched_items' not in interface.keys():
            interface['max_matched_items'] = UserInterfaceManager.DEFAULT_MAX_MATCHED_ITEMS
        else:
            try:
                interface['max_matched_items'] = int(float(interface['max_matched_items']))
            except ValueError:
                interface['max_matched_items'] = UserInterfaceManager.DEFAULT_MAX_MATCHED_ITEMS
