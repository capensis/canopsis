...
def transform_event(ws, am, event):
    ...
    if 'author' in new_event and new_event['author']:
        user_crecord_name = lookup_user(new_event['author'])
        if user_crecord_name:
            new_event['author'] = user_crecord_name
...
def lookup_user(id):
    rights = get_rights()
    user = rights.get_user(id)

    if user:
        return user.get('crecord_name')
