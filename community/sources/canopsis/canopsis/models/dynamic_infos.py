...
class DynamicInfo(object):
    ...
    :param any value: The value or the information
    """
    NAME = 'name'
    VALUE = 'value'
    FIELDS = frozenset([
    ...
    def _check_valid(self):
        ...
        if not (self.value is None or isinstance(self.value, basestring) or isinstance(
            self.value, int) or isinstance(self.value, bool)):
            raise TypeError(
                "{} unsupported type {}".format(DynamicInfo.VALUE, type(self.value)))

    def as_dict(self):
        """Return the DynamicInfo as a dictionary that can be stored in
        MongoDB.
