...
```sh
sed -i "s/^#\?timezone.*/timezone = 'UTC'/" /var/lib/pgsql/15/data/postgresql.conf
sed -i "s/^#\?log_timezone.*/log_timezone = 'UTC'/" /var/lib/pgsql/15/data/postgresql.conf
```
...
=== "Canopsis Community (édition open-source)"
    ...
    dnf versionlock add --raw 'canopsis-common-25.04.*'
