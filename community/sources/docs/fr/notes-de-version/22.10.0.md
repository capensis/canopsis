...
Canopsis 22.10.0 a été publié le 4 novembre 2022.
...
Il est également à présent possible de filter les alarmes dont une remédiation est en cours ou non.
...
Il s'agit de pouvoir traiter des `tags` en provenances des sources de données.  
...
Un ouveau widget fait son apparition, le widget `Cartographie` ou `Map`.  
