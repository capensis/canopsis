...
En édition Pro, si vous utilisez les moteurs `engine-webhook` et `engine-dynamic-infos`, vous pouvez simplifier les points d'entrée en supprimant les paramètres `-publishQueue`, si vous les aviez exactement positionnés aux valeurs suivantes :
...
Pour Redis, notez qu'il est **fort probable** que vous ayez besoin d'ajuster sa variable `maxmemory`. Voyez pour cela la [documentation sur le service Redis](../guide-administration/administration-avancee/configuration-services/serveur-cache-redis.md).
...
Vous devez ensuite vous [connecter à l'interface de RabbitMQ](../guide-de-depannage/rabbitmq-webui.md) pour y apporter quelques changements.
