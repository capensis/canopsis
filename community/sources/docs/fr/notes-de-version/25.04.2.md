# Notes de version Canopsis 25.04.2

Canopsis 25.04.2 a été publié le 11 septembre 2025.

## Procédure d'installation

Suivre la [procédure d'installation de Canopsis](../guide-administration/installation/index.md).

## Procédure de mise à jour

Suivre la [procédure standard de mise à jour de Canopsis](../guide-administration/mise-a-jour/index.md).

## Changements entre Canopsis 25.04.1 et 25.04.2

### Améliorations

*  **Interface graphique :**
    * le type d'un rôle ne peut à présent plus être modifié (#5848)
    * Les utilisateurs avec uniquement un rôle "API" ne peuvent plus se logguer sur l'interface graphique (#5953)
    * **Bac à alarmes :**
        * L'action "Changer et verrouiller la criticité" est disponible en action de masse (#5940)
    * **Règles de génération de liens :**
        * Le label d'une règle de génération de liens peut à présent utiliser un template (#5925)

*  **API :**
    * Optimisation des endpoints de création de masse des comportements périodiques (#5795)
    * Ajout de la fonction de pagination sur les tags, évitant ainsi des lenteurs sur l'interface dûes à des slow queries en base de données (#5951)
    * L'API Canopsis reste désormais opérationnelle même en cas d'indisponibilité temporaire de Redis (#5903)
    * Ajout de messages de log websocket permettant d'améliorer la phase de debug (#5879)

*  **Authentification :**
    * Oauth2 : Le paramètre insecure_skip_verify n’accepte plus que les certificats valides et auto-signés. Pour accepter tout type de certificat (y compris expirés ou invalides), utilisez désormais insecure_verify_any_cert: true (#5955)

*  **Export Prometheus :**
    * De nouvelles métriques sont disponibles : alarmes actives/inactives, messages FIFO par type, entités actives par type et nombre de règles d’instructions (#5853)

### Corrections de bugs

*  **Interface graphique :**
    * Correction d'un bug qui rendait inopérante la duplication d'un rôle (#5964)
    * Correction d'un bug qui empêchait la création d'une règle de filtrage avec des données externes (#5946)
    * Les étapes d'une remédiation peuvent à nouveau être reordonnées par drag'n drop (#5960)
    * Correction d'un bug qui empêchait la saisie d'un pattern (Consigne et Règles de résolution) s'il avait été oublié en première intention (#5961)
