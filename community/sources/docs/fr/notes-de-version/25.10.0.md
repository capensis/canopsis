...
Canopsis introduit la gestion des jetons d'authentification externe (external auth tokens).  
Cette fonctionnalité permet de configurer l'obtention, le stockage et la réutilisation automatique de jetons délivrés par des services tiers (API REST, outils de ticketing, CMDB, etc.) pour les réutiliser dans les scénarios ou les règles de déclaration de tickets.

Principe :

* Canopsis envoie une requête HTTP configurée (méthode, URL, entêtes, payload) vers un service d'authentification externe.
* Le jeton est extrait automatiquement depuis la réponse (texte brut ou champ JSON).
* Le temps d'expiration du jeton est défini afin que Canopsis sache quand le renouveler.
* Tant qu'il est valide, le jeton est injecté automatiquement dans les requêtes sortantes (ex. Authorization: Bearer {{ .Token }}).
...
* Options des moteurs deprecated
