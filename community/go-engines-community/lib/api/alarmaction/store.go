...
type Store interface {
	...
	Comment(ctx context.Context, id string, r Request, userId, username string) (bool, error)
...
func (s *store) Comment(ctx context.Context, id string, r Request, userId, username string) (bool, error) {
	alarm, err := s.findAlarm(ctx, bson.M{"_id": id})
	if err != nil || alarm.Alarm.ID == "" {
		return false, err
	}
