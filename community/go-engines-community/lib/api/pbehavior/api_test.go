...
func init() {
	dbClient, err := mongo.NewClient(context.Background(), 0, 0, zerolog.Nop())
	if err != nil {
		panic(err)
	}

	libapi.RegisterValidators(dbClient)
}
...
func benchmarkBulkConnectorEdit_givenNCreateItems(b *testing.B, itemCount int) {
	...
	authorProvider := author.NewProvider(dbClient, &config.BaseApiConfigProvider{})
	store := pbehavior.NewStore(dbClient, nil, nil, nil, authorProvider)
	api := pbehavior.NewApi(store, ch, nil, nil, zerolog.Nop())
