...
// detailsFixture is the fixture file produced by alarm-fixture-gen.
...
func detailsRequest(reversed, group bool, page int64) alarm.DetailsRequest {
	opened := true
	r := alarm.DetailsRequest{
		ID:                 benchAlarmID,
		Opened:             &opened,
		WithInstructions:   true,
		WithDeclareTickets: true,
		WithDependencies:   true,
		Steps: &alarm.StepsRequest{
			Query: pagination.Query{
				Page:  page,
				Limit: 10,
			},
		...
		Children: &alarm.ChildDetailsRequest{
			Query: pagination.Query{
				Page:  1,
				Limit: 10,
			},
			SortRequest: alarm.SortRequest{
...
func benchmarkStoreGetDetails(b *testing.B, fixturesPath string, request alarm.DetailsRequest) {
	b.Helper()
	ctx := b.Context()
...
func getAlarmStoreWithFixtures(ctx context.Context, b *testing.B, fixturesPath string) (alarm.Store, error) {
