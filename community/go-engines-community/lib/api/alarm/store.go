...
type Store interface {
	Find(string, ListRequestWithPagination) (*AggregationResult, error)
	Count(FilterRequest) (*Count, error)
}
...
func (s *store) adjustSort(sort bson.M, project []bson.M) []bson.M {
	srt := sort["$sort"]
	if sortExpr, ok := srt.(bson.M); ok && sortExpr != nil && sortExpr["v.duration"] != nil {
...
func (s *store) Find(apiKey string, r ListRequestWithPagination) (*AggregationResult, error) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	pipeline := make([]bson.M, 0)
	s.addNestedObjects(r.FilterRequest, &pipeline)
	err := s.addFilter(r.FilterRequest, &pipeline)
	if err != nil {
		return nil, err
	}
	...
	err = s.fillAssignedInstructions(&result)
	if err != nil {
		return nil, err
	}
...
func (s *store) Count(r FilterRequest) (*Count, error) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	pipeline := make([]bson.M, 0)
	s.addNestedObjects(r, &pipeline)
	err := s.addFilter(r, &pipeline)
	if err != nil {
		return nil, err
	}
...
func (s *store) getAssignedInstructionsMap(alarmIds []string) (map[string][]InstructionWithAlarms, error) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	pipeline := []bson.M{
		...
		{"$lookup": bson.M{
			"from":         entity.EntityCollectionName,
			"localField":   "d",
			"foreignField": "_id",
			"as":           "entity",
		}},
...
func (s *store) fillAssignedInstructions(result *AggregationResult) error {
	var alarmIds []string
	...
	assignedInstructionsMap, err := s.getAssignedInstructionsMap(alarmIds)
	if err != nil {
		return err
	}
...
func (s *store) addFilter(r FilterRequest, pipeline *[]bson.M) error {
	match := make([]bson.M, 0)
	...
	err = s.addOnlyInstructionsFilter(r, &match)
	if err != nil {
		return err
	}
...
func (s *store) addOnlyInstructionsFilter(r FilterRequest, match *[]bson.M) error {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func (s *store) getProject(r ListRequest, entitiesToProject bool) []bson.M {
	...
	if entitiesToProject {
		pipeline = append(pipeline,
			bson.M{"$lookup": bson.M{"as": "entity", "foreignField": "_id", "from": entity.EntityCollectionName, "localField": "d"}},
			bson.M{"$unwind": bson.M{"path": "$entity", "preserveNullAndEmptyArrays": true}},
		)
	}
