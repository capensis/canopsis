...
type StatsStore interface {
	...
	SetStats(context.Context, string, Stats) error
	ResetStats(context.Context, string) error
}

// NewStatsStore instantiates a StatsStore
func NewStatsStore(dbClient mongo.DbClient, location *time.Location) StatsStore {
	return &statsStore{
...
func (s *statsStore) GetStats(ctx context.Context, eid string) (result Stats, err error) {
	...
	nowYear, nowMonth, nowDay := time.Now().In(s.location).Date()
	prevYear, prevMonth, prevDay := result.LastEvent.Time.In(s.location).Date()
...
func (s *statsStore) SetStats(ctx context.Context, eid string, st Stats) error {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	...
	if prev.LastEvent != nil {
		...
		evtYear, evtMonth, evtDay := lastEventTime.In(s.location).Date()
		prevYear, prevMonth, prevDay := prev.LastEvent.Time.In(s.location).Date()
