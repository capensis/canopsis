...
func NewEngine(
	...
	engineAxe.AddPeriodicalWorker("alarms", libengine.NewLockedPeriodicalWorker(
		...
		&periodicalWorker{
			...
			CancelDelayService: canceldelay.NewService(
				dbClient,
				eventGenerator,
				options.PeriodicalWaitTime,
			),
...
func (m DependencyMaker) EventProcessor(
	...
	container.Set(types.EventTypeCancelDelay, event.NewCancelDelayProcessor(dbClient, metaAlarmPostProcessor, logger))
