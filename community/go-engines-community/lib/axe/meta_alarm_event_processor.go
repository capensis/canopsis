...
func (p *metaAlarmEventProcessor) CreateMetaAlarm(ctx context.Context, event rpc.AxeEvent) (*types.Alarm, []types.Alarm, error) {
	...
	err := p.dbClient.WithTransaction(ctx, func(ctx context.Context) error {
		...
		metaAlarm.Value.LastEventDate = libtime.CpsTime{} // should be empty
...
func (p *metaAlarmEventProcessor) DetachChildrenFromMetaAlarm(ctx context.Context, event rpc.AxeEvent) (*types.Alarm, error) {
	...
	err = p.dbClient.WithTransaction(ctx, func(ctx context.Context) error {
		...
		var lastEventDate libtime.CpsTime // should be empty
		worstState := types.CpsNumber(types.AlarmStateOK)
...
func (p *metaAlarmEventProcessor) resolveParents(ctx context.Context, childAlarm types.Alarm, timestamp libtime.CpsTime) error {
	ch := make(chan string)
		...
		g.Go(func() error {
			for parentId := range ch {
				...
				err := p.dbClient.WithTransaction(ctx, func(ctx context.Context) error {
					...
					err = parentAlarm.Alarm.PartialUpdateResolve(time.NewCpsTime())
					if err != nil {
			...
			for parentId := range ch {
				err := p.dbClient.WithTransaction(ctx, func(ctx context.Context) error {
					...
					if childState > parentState {
						...
						parentAlarm.Alarm.PartialUpdateLastEventDate(time.NewCpsTime(lastEventDate))
					} else {
						return nil
					}
...
func updateMetaAlarmState(alarm *types.Alarm, entity types.Entity, timestamp libtime.CpsTime, state types.CpsNumber, output string,
