...
func TestService_Process_GivenAlarmRuleByLastEventDate_ShouldReturnEvent(t *testing.T) {
	...
	alarm := types.Alarm{
		...
		Value: types.AlarmValue{
			LastEventDate: libtime.CpsTime{Time: time.Now().Add(-6 * time.Hour)},
	...
	rule := idlerule.Rule{
		...
		Duration: libtime.DurationWithUnit{
			Value: 10,
			Unit:  "s",
		},
	...
	event := events[0]
	event.Timestamp = libtime.CpsTime{}
...
func TestService_Process_GivenAlarmRuleByLastUpdateDate_ShouldReturnEvent(t *testing.T) {
	...
	alarm := types.Alarm{
		ID: "test-alarm",
		Value: types.AlarmValue{
			LastUpdateDate: libtime.CpsTime{Time: time.Now().Add(-6 * time.Hour)},
			Connector:      "test-connector",
			ConnectorName:  "test-connector-name",
			Component:      "test-component",
			Resource:       "test-resource",
