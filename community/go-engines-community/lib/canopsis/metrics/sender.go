...
func (s *sender) HandleMetricsByEvent(event types.Event) {
	...
	case types.AlarmChangeTypeCreate:
		s.sendMetric(Event{
			...
			Labels: prometheus.Labels{"entityID": alarm.EntityID, "category": entity.Category},
		})
	...
	case types.AlarmChangeTypeAssocTicket:
		...
		s.sendMetric(Event{
			...
			Labels: prometheus.Labels{"entityID": alarm.EntityID, "category": entity.Category, "username": event.Author},
		})

		s.sendMetric(Event{
			Type:   TicketAlarmNumberSlice,
			Labels: prometheus.Labels{"slice": DefaultSlice, "username": event.Author},
		})
