...
type service struct {
	...
	statsStore      serviceweather.StatsStore
...
func (s *service) setStats(
	...
	return s.statsStore.SetStats(ctx, entityID, stats, location)
}

// ProcessAlarmChange processes an AlarmChange, and sends the corresponding
// statistic events to the statsng engine.
func (s *service) ProcessAlarmChange(
	ctx context.Context,
	change types.AlarmChange, timestamp types.CpsTime,
	alarm types.Alarm,
	entity types.Entity,
	...
	if change.Type == types.AlarmChangeTypePbhEnter {
		...
		err = s.setStats(ctx, timestamp, alarm.Value.State, entity.ID, location)
		if err != nil {
