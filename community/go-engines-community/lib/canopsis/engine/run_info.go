...
func (m *runInfoManager) Save(info RunInfo, expiration time.Duration) error {
	if info.Name == "" {
	...
	res := m.client.Set(m.getKey(info.Name), b, expiration)
...
func (m *runInfoManager) Get(engineName string) (*RunInfo, error) {
	if engineName == "" {
		return nil, errors.New("name is required")
	}

	res := m.client.Get(m.getKey(engineName))
	if err := res.Err(); err != nil {
		if err == redis.Nil {
			return nil, nil
		}
...
func (m *runInfoManager) GetAll() ([]RunInfo, error) {
	infos := make([]RunInfo, 0)
	var cursor uint64
	processedKeys := make(map[string]bool)

	for {
		res := m.client.Scan(cursor, fmt.Sprintf("%s*", m.key), 50)
		if err := res.Err(); err != nil {
			return nil, err
		}
		...
		if len(unprocessedKeys) > 0 {
			resGet := m.client.MGet(unprocessedKeys...)
...
func (m *runInfoManager) GetGraph() (*RunInfoGraph, error) {
	infos, err := m.GetAll()
	if err != nil {
		return nil, err
	}
...
func (m *runInfoManager) ClearAll() error {
	var cursor uint64
	for {
		res := m.client.Scan(cursor, fmt.Sprintf("%s*", m.key), 50)
		if err := res.Err(); err != nil {
