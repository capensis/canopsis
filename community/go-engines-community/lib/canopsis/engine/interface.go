...
//go:generate mockgen -destination=../../../mocks/lib/canopsis/engine/engine.go git.canopsis.net/canopsis/go-engines/lib/canopsis/engine Engine,Consumer,MessageProcessor,PeriodicalWorker,RPCClient,RPCMessageProcessor
...
type MessageProcessor interface {
	Process(d amqp.Delivery) (newMessage []byte, err error)
}
...
type RunInfoManager interface {
	Save(info RunInfo, expiration time.Duration) error
	Get(engineName string) (*RunInfo, error)
	GetAll() ([]RunInfo, error)
	GetGraph() (*RunInfoGraph, error)
	ClearAll() error
}
