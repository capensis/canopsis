...
func (eng *testEngineBadConsumerChan) WorkerProcess(ctx context.Context, msg amqp.Delivery) {
}
...
func (eng *testEngineBadConsumerChan) PeriodicalProcess(ctx context.Context) {
...
func (eng *testEngineInitializeFailed) Initialize(ctx context.Context) error {
...
func (eng *testEngine) PeriodicalProcess(ctx context.Context) {
	eng.Logger().Info().Msg("PERIODICAL: working 1sec...")
	time.Sleep(time.Second)
	eng.Logger().Info().Msg("PERIODICAL: work finished")
}

func (eng *testEngine) WorkerProcess(ctx context.Context, msg amqp.Delivery) {
...
func TestDefaultEngine(t *testing.T) {
	...
	Convey("Test different engines that MUST NOT PANIC", t, func() {
		...
		Convey("Test an engine with worker and periodical", func() {
			...
			exitStatus, err := cps.StartEngine(ctx, &engine, &waitChan)
		...
		Convey("Consumer failed", func() {
			...
			stopAfter2Sec()
			exitStatus, err := cps.StartEngine(ctx, &engine, nil)
			So(exitStatus, ShouldEqual, cps.ExitEngine)
...
func (eng *WorkerPanicEngine) PeriodicalProcess(ctx context.Context) {
	eng.Logger().Info().Msg("periodical coucou")
}

func (eng *WorkerPanicEngine) WorkerProcess(ctx context.Context, msg amqp.Delivery) {
...
func TestWorkerPanicEngine(t *testing.T) {
	...
	Convey("Setup panic engine on worker process", t, func() {
		...
		startEngine := func() {
			cps.StartEngine(context.Background(), &pe, nil)
		}
...
func (eng *PeriodicalPanicEngine) PeriodicalProcess(ctx context.Context) {
	panic("lavitesseetlefundabord")
}

func (eng *PeriodicalPanicEngine) WorkerProcess(ctx context.Context, msg amqp.Delivery) {
	if string(msg.Body) == "panic" {
		panic("jepanikay")
	} else {
		eng.Logger().Info().Msg(string(msg.Body))
