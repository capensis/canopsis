...
func (p EnrichmentRule) getExternalData(ctx context.Context, parameters DataSourceGetterParameters, report *Report) (map[string]interface{}, error) {
	...
	for name, source := range p.ExternalData {
		...
		data, err := source.Get(ctx, parameters, report)
		if err != nil {
...
func (p EnrichmentRule) Apply(ctx context.Context, event types.Event, regexMatches pattern.EventRegexMatches, report *Report, logger zerolog.Logger, ruleId string) (types.Event, Outcome) {
	...
	externalData, err := p.getExternalData(ctx, parameters, report)
	if err != nil {
