...
func (r Rule) Apply(event types.Event, regexMatches pattern.EventRegexMatches, report *Report, logger zerolog.Logger) (types.Event, Outcome) {
	switch r.Type {
	case RuleTypeBreak:
		return event, Break
	case RuleTypeDrop:
	...
	case RuleTypeEnrichment:
		return r.EnrichmentRule.Apply(event, regexMatches, report, logger, r.ID)
	default:
