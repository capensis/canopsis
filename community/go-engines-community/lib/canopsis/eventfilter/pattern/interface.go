	...
	case bsontype.EmbeddedDocument:
		...
		if len(p.IntegerConditions.UnexpectedFields) != 0 &&
			...
			return UnexpectedFieldsError{
				Err: fmt.Errorf("interface patterns should only contain conditions on a string (regex_match) or conditions on an integer (>, <, >=, <=) or conditions on a string array(has_every, has_one_of, has_not); those three types of conditions cannot be mixed"),
			}
		}
		return nil
	default:
