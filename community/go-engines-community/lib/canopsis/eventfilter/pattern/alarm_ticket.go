...
func (p AlarmTicketRefPattern) AsMongoQuery(prefix string, query mgobson.M) {
	if p.ShouldBeNil {
		query[prefix] = nil
		return
	}
	...
	if len(p.Data) > 0 {
		for k, v := range p.Data {
			query[fmt.Sprintf("%s.data.%s", prefix, k)] = v.AsMongoQuery()
		}
	}
}

func (p AlarmTicketRefPattern) AsMongoDriverQuery(prefix string, query mongobson.M) {
	if p.ShouldBeNil {
		query[prefix] = nil
		return
	}
...
func (p *AlarmTicketRefPattern) SetBSON(raw mgobson.Raw) error {
	switch raw.Kind {
	...
	default:
		err := raw.Unmarshal(&p.AlarmTicketFields)
		if err != nil {
			return err
		}
...
func (p *AlarmTicketRefPattern) UnmarshalBSONValue(valueType bsontype.Type, b []byte) error {
	...
	case bsontype.EmbeddedDocument:
		err := mongobson.Unmarshal(b, &p.AlarmTicketFields)
		if err != nil {
			return err
		}
		if len(p.UnexpectedFields) != 0 {
