...
// service.
func (s *service) LoadDataSourceFactories(enrichmentCenter context.EnrichmentCenter, enrichFields context.EnrichFields, dataSourceDirectory string) error {
...
func (s *service) ProcessEvent(event types.Event) (types.Event, Report, error) {
	s.rulesMutex.RLock()
	...
	for _, rule := range s.rules {
		...
		if match {
			...
			event, outcome = rule.Apply(event, regexMatches, &report, s.logger)
