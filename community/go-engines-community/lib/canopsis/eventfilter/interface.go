...
type ActionProcessor interface {
	Process(ctx context.Context, ruleID string, action ParsedAction, event types.Event, regexMatch RegexMatch, externalData map[string]interface{}) (types.Event, error)
}
...
type RuleApplicator interface {
	...
	Apply(context.Context, ParsedRule, types.Event, RegexMatch) (string, types.Event, error)
}
