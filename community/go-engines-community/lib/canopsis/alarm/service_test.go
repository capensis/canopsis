...
func TestService_ResolveCancels(t *testing.T) {
	...
	var dataSets = []struct {
		...
		},
		{
			...
			[]types.Alarm{
				...
				newCancelAlarm(types.CpsTime{
					Time: time.Now().Add(-cps.CancelAutosolveDelay * time.Second),
				}),
				newCancelAlarm(types.CpsTime{
					Time: time.Now().Add(-cps.CancelAutosolveDelay * time.Second),
				}),
	...
	for _, dataset := range dataSets {
		t.Run(dataset.testName, func(t *testing.T) {
			...
			cancelAlarms, err := service.ResolveCancels(context.Background(), time.Minute*60)
			if err != nil {
...
func TestService_ResolveAlarms(t *testing.T) {
	...
	for _, dataset := range dataSets {
		t.Run(dataset.testName, func(t *testing.T) {
			...
			closedAlarms, err := service.ResolveAlarms(context.Background(), baggotTime)
			if err != nil {
...
func TestService_ResolveSnoozes(t *testing.T) {
	...
	for _, dataset := range dataSets {
		t.Run(dataset.testName, func(t *testing.T) {
			...
			unsnoozedAlarms, err := service.ResolveSnoozes(context.Background(), false)
			if err != nil {
