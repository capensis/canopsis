...
type Adapter interface {
	...
	UpdateLastEventDate(ctx context.Context, ids []string, time libtime.CpsTime) error
	UpdateIdleFields(ctx context.Context, id string, idleSince *libtime.CpsTime, lastIdleRuleApply string) error

	FindByIDs(ctx context.Context, ids []string) ([]types.Entity, error)

	GetAllWithLastUpdateDateBefore(ctx context.Context, time libtime.CpsTime, exclude []string) (mongo.Cursor, error)
