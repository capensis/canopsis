...
func (s *ruleService) LoadRules() error {
	...
	for _, rule := range rules {
		err := s.ruleEntityCounter.CountTotalEntitiesAmount(rule)
		if err != nil {
			return err
		}

		err = s.valueGroupEntityCounter.CountTotalEntitiesAmount(rule)
		if err != nil {
			return err
		}
	}
...
func (s *ruleService) ProcessEvent(event *types.Event) ([]types.Event, error) {
	...
	if event.EventType == types.EventManualMetaAlarmGroup || event.EventType == types.EventManualMetaAlarmUpdate ||
		...
		events, err := applicator.Apply(event, Rule{})
		if err != nil {
	...
	for _, rule := range s.rules {
		...
		if isMatched {
			...
			if !found {
				...
				metaAlarmEvent, err := applicator.Apply(event, rule)
				if err != nil {
					s.logger.Err(err).
						Str("rule_id", rule.ID).
...
func (s *ruleService) isNotFound(err error) bool {
