...
func (a ComplexApplicator) Apply(event *types.Event, rule metaalarm.Rule) ([]types.Event, error) {
		...
		for redisRetries := MaxRedisRetries; redisRetries >= 0; redisRetries-- {
			watchErr = a.redisClient.Watch(func(tx *redis.Tx) error {
				...
				if groupLen >= *rule.Config.ThresholdCount {
					...
					err = a.storage.Push(tx, rule, *event.Alarm, "")
					if err != nil {
						return err
					}
	...
	if rule.Config.ThresholdRate != nil {
		for redisRetries := MaxRedisRetries; redisRetries >= 0; redisRetries-- {
			watchErr = a.redisClient.Watch(func(tx *redis.Tx) error {
				ratioReached, err := a.isRatioReached(tx, rule, true)
				if err != nil {
					return err
				}
				...
				if ratioReached {
					...
					ratioReached, err = a.isRatioReached(tx, rule, false)
					if err != nil {
						return err
					}
...
func (a ComplexApplicator) isRatioReached(tx *redis.Tx, rule metaalarm.Rule, includeNewAlarmOnRecompute bool) (bool, error) {
	groupLen, err := a.getGroupLen(tx, rule.ID)
	if err != nil {
		return false, err
	}

	total, err := a.ruleEntityCounter.GetTotalEntitiesAmount(rule)
	if err != nil {
		return false, err
	}

	if total != 0 && float64(groupLen)/float64(total) >= *rule.Config.ThresholdRate {
		total, err = a.recomputeTotal(rule)
		if err != nil {
			return false, err
		}
...
func (a ComplexApplicator) recomputeTotal(rule metaalarm.Rule) (int, error) {
	err := a.ruleEntityCounter.CountTotalEntitiesAmount(rule)
	if err != nil {
		return 0, err
	}
...
func (a ComplexApplicator) createMetaAlarm(tx *redis.Tx, event *types.Event, rule metaalarm.Rule) (types.Event, error) {
...
func (a ComplexApplicator) getGroupLen(tx *redis.Tx, ruleId string) (int64, error) {
	var children []types.AlarmWithEntity

	alarmGroup, err := a.storage.Get(tx, ruleId)
	if err != nil {
		return 0, err
	}
