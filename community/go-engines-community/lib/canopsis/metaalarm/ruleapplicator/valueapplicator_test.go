...
func testNewValueApplicator() (*ValueApplicator, alarm.Adapter, entity.Adapter, error) {
	...
	valueGroupEntityCounter := metaalarm.NewValueGroupEntityCounter(mongo, redisClient3, logger)
...
func TestApply(t *testing.T) {
	...
	Convey("Test valuegroup rule match", t, func() {
		...
		Convey("check simple valuegroup rule", func() {
			...
			metaAlarmEventArray, _ := applicator.Apply(&testEvent1, rule)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent2, rule)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent3, rule)
			...
			metaAlarmEventArray, _ = applicator.Apply(&testEvent4, rule)
			So(metaAlarmEventArray, ShouldNotBeNil)
			So(len(metaAlarmEventArray), ShouldEqual, 1)
			metaAlarmEvent = metaAlarmEventArray[0]
			So(metaAlarmEvent.MetaAlarmRuleID, ShouldEqual, "valuegroup-test")
			So(metaAlarmEvent.MetaAlarmValuePath, ShouldEqual, "customer-1.location-2")
			metaAlarmEventArray, _ = applicator.Apply(&testEvent5, rule)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent6, rule)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent7, rule)
			So(metaAlarmEventArray, ShouldNotBeNil)
			So(len(metaAlarmEventArray), ShouldEqual, 1)
			metaAlarmEvent = metaAlarmEventArray[0]
			So(metaAlarmEvent.MetaAlarmRuleID, ShouldEqual, "valuegroup-test")
			So(metaAlarmEvent.MetaAlarmValuePath, ShouldEqual, "customer-2.location-1")
			metaAlarmEventArray, _ = applicator.Apply(&testEvent8, rule)
			So(metaAlarmEventArray, ShouldNotBeNil)
			So(len(metaAlarmEventArray), ShouldEqual, 1)
			metaAlarmEvent = metaAlarmEventArray[0]
			So(metaAlarmEvent.MetaAlarmRuleID, ShouldEqual, "valuegroup-test")
...
func TestApplyWithRate(t *testing.T) {
	...
	Convey("Init data", t, func() {
		...
		valueGroupEntityCounter := metaalarm.NewValueGroupEntityCounter(mongo, redisClient, logger)
		err = valueGroupEntityCounter.CountTotalEntitiesAmount(rule1)
		So(err, ShouldBeNil)
		total, err := valueGroupEntityCounter.GetTotalEntitiesAmount(rule1.ID, "customer-1.location-1")
		So(err, ShouldBeNil)
		So(total, ShouldEqual, 4)
		total, err = valueGroupEntityCounter.GetTotalEntitiesAmount(rule1.ID, "customer-1.location-2")
		So(err, ShouldBeNil)
		So(total, ShouldEqual, 4)
		err = valueGroupEntityCounter.CountTotalEntitiesAmount(rule2)
		So(err, ShouldBeNil)
		total, err = valueGroupEntityCounter.GetTotalEntitiesAmount(rule2.ID, "customer-1.location-1")
		So(err, ShouldBeNil)
		So(total, ShouldEqual, 4)
		total, err = valueGroupEntityCounter.GetTotalEntitiesAmount(rule2.ID, "customer-1.location-2")
		So(err, ShouldBeNil)
		So(total, ShouldEqual, 4)
		...
		Convey("check simple valuegroup rule1, rate should be reached - 3/4 > 0.7", func() {
			...
			metaAlarmEventArray, _ := applicator.Apply(&testEvent1, rule1)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent2, rule1)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent3, rule1)
			So(metaAlarmEventArray, ShouldNotBeNil)
			So(len(metaAlarmEventArray), ShouldEqual, 1)
			metaAlarmEvent := metaAlarmEventArray[0]
			So(metaAlarmEvent.MetaAlarmRuleID, ShouldEqual, "valuegroup-test-rate-1")
			So(metaAlarmEvent.MetaAlarmValuePath, ShouldEqual, "customer-1.location-1")
			metaAlarmEventArray, _ = applicator.Apply(&testEvent5, rule1)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent6, rule1)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent7, rule1)
			So(len(metaAlarmEventArray), ShouldEqual, 1)
			metaAlarmEvent = metaAlarmEventArray[0]
			So(metaAlarmEvent.MetaAlarmRuleID, ShouldEqual, "valuegroup-test-rate-1")
			So(metaAlarmEvent.MetaAlarmValuePath, ShouldEqual, "customer-1.location-2")
		...
		Convey("check simple valuegroup rule1, rate shouldn't be reached - 3/4 < 0.8", func() {
			...
			metaAlarmEventArray, _ := applicator.Apply(&testEvent1, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent2, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent3, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent5, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent6, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
			metaAlarmEventArray, _ = applicator.Apply(&testEvent7, rule2)
			So(metaAlarmEventArray, ShouldBeNil)
		})
	})
}
