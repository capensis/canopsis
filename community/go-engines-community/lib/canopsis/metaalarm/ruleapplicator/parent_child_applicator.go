...
func (a ParentChildApplicator) Apply(event *types.Event, rule metaalarm.Rule) ([]types.Event, error) {
	...
	if event.SourceType == types.SourceTypeComponent {
		...
		if !event.Alarm.IsMetaAlarm() {
			...
			if len(resourceAlarms) != 0 {
				...
				metaAlarmLock, err = a.redisLockClient.Obtain(componentAlarm.ID, 100*time.Millisecond, &redislock.Options{
