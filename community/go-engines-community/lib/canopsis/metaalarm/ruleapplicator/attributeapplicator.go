...
func (a AttributeApplicator) Apply(event *types.Event, rule metaalarm.Rule) ([]types.Event, error) {
	...
	for redisRetries := MaxRedisRetries; redisRetries >= 0; redisRetries-- {
		watchErr = a.redisClient.Watch(func(tx *redis.Tx) error {
			...
			group, err := a.storage.Get(tx, rule.ID)
			if err != nil {
				return err
			}
...
func (a AttributeApplicator) AlarmMatched(event types.Event, rule metaalarm.Rule) bool {
	...
	a.logger.Debug().Msgf("Alarm matched event %+v with rule %v %t", event, rule.Config.AlarmPatterns.AsMongoQuery(), patternsMatch)
...
func (a AttributeApplicator) EntityMatched(event types.Event, rule metaalarm.Rule) bool {
	...
	a.logger.Debug().Msgf("Entity matched event %+v with rule %v %t", event, rule.Config.EntityPatterns.AsMongoQuery(), patternsMatch)
	return patternsMatch
}
