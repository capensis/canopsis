...
// NewMetaAlarmService instantiates meta-alarm service; receives entityAdapter as adapter to Entity collection
// and alarmAdapter as adapter to db Alarm collection
func NewMetaAlarmService(
	alarmAdapter alarm.Adapter, entityAdapter entity.Adapter, ruleApt metaalarm.RulesAdapter,
	cfg config.CanopsisConf, logger zerolog.Logger) MetaAlarmService {
	return &service{
...
func (s *service) AddChildToMetaAlarm(
	...
	if childAlarm.Value.State != nil {
		...
		if childAlarm.Value.State.Value > maCurrentState {
			...
			alarm.UpdateToWorstState(&metaAlarm, []*types.Alarm{&childAlarm}, s.alarmAdapter, s.cfg)
		}
	}
...
func (s *service) AddMultipleChildsToMetaAlarm(
	...
	if worstState > maCurrentState {
		...
		alarm.UpdateToWorstState(&metaAlarm, updateChildren, s.alarmAdapter, s.cfg)
	}
