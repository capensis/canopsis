...
import (
	...
	mock_entity "git.canopsis.net/canopsis/go-engines/mocks/lib/canopsis/entity"
...
func TestAddChildToMetaAlarmWorstState(t *testing.T) {
	...
	entityAdapterMock := mock_entity.NewMockAdapter(ctrl)
	...
	s := service.NewMetaAlarmService(alarmAdapterMock, entityAdapterMock, rulesAdapter, testutils.GetTestConf(), log.NewTestLogger())
...
func TestUpdateChildToMetaAlarmDecreaseWorstState(t *testing.T) {
	...
	alarmAdapterMock.
		EXPECT().
		GetOpenedAlarmsByIDs(childrenEIDs, &openedAlarms).
		DoAndReturn(func(_ []string, c *[]types.Alarm) error {
...
func getRuleAdapter() (metaalarm.RulesAdapter, error) {
	mgo, er := mongo.NewSession(mongo.Timeout)
	if er != nil {
		return nil, er
	}
	rulesCollection := metaalarm.DefaultRulesCollection(mgo)
	_, er = rulesCollection.RemoveAll()
	if er != nil {
		return nil, er
	}
