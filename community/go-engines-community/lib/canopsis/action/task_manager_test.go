...
func TestTaskManager_Run_GiveTask_ShouldSendResult(t *testing.T) {
	...
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Eq(int64(1)), gomock.Eq(true)).Return(int64(1), nil)
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Eq(int64(-1)), gomock.Eq(false)).Return(int64(0), nil)
	mockExecutionStorage.EXPECT().Create(gomock.Any()).Return(executionID, nil)
	mockExecutionStorage.EXPECT().Get(executionID).Return(&execution, nil)
	mockExecutionStorage.EXPECT().Update(gomock.Any()).Return(nil)
	mockExecutionStorage.EXPECT().Del(executionID).Return(nil)
	...
	mockScenarioStorage.EXPECT().
		RunDelayedScenarios(gomock.Eq(task.Triggers), gomock.Eq(task.Alarm), gomock.Eq(task.Entity)).
		Return(nil)
...
func TestTaskManager_Run_GiveTaskWithEmitTrigger_ShouldSendResult(t *testing.T) {
	...
	mockWorkerPool.EXPECT().RunWorkers(gomock.Any(), gomock.Any()).
		...
		Return(taskResultCh, nil)
	mockExecutionStorage := mock_action.NewMockScenarioExecutionStorage(ctrl)
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Eq(int64(1)), gomock.Eq(true)).
		Return(int64(1), nil).Times(1)
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Eq(int64(1)), gomock.Eq(false)).
		Return(int64(1), nil).Times(1)
	decCall := int64(2)
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Eq(int64(-1)), gomock.Eq(false)).
		DoAndReturn(func(_ string, _ int64, _ bool) (int64, error) {
			decCall--
			return decCall, nil
		}).Times(2)
	createCall := 0
	mockExecutionStorage.EXPECT().Create(gomock.Any()).DoAndReturn(func(_ action.ScenarioExecution) (string, error) {
		createCall++
		return fmt.Sprintf("test-alarm&&test-scenario-%d", createCall), nil
	}).Times(2)
	mockExecutionStorage.EXPECT().Get(firstExecutionID).Return(&firstExecution, nil)
	mockExecutionStorage.EXPECT().Get(secondExecutionID).Return(&secondExecution, nil)
	mockExecutionStorage.EXPECT().Update(gomock.Any()).Return(nil).Times(2)
	mockExecutionStorage.EXPECT().Del(firstExecutionID).Return(nil)
	mockExecutionStorage.EXPECT().Del(secondExecutionID).Return(nil)
	mockScenarioStorage := mock_action.NewMockScenarioStorage(ctrl)
	mockScenarioStorage.EXPECT().
		GetTriggeredScenarios(gomock.Eq(task.Triggers), gomock.Eq(task.Alarm)).
	...
	mockScenarioStorage.EXPECT().
		...
		Return(nil)
	mockScenarioStorage.EXPECT().
		RunDelayedScenarios(gomock.Eq([]string{firstScenario.Actions[0].Type}), gomock.Eq(task.Alarm), gomock.Eq(task.Entity)).
		Return(nil)
	logger := zerolog.Logger{}
	manager := action.NewTaskManager(mockWorkerPool, mockExecutionStorage, mockScenarioStorage, logger)
	resultCh, err := manager.Run(ctx, rpcResultCh, inputCh)
...
func TestTaskManager_Run_GiveAbandonedTask_ShouldSendResult(t *testing.T) {
	...
	mockWorkerPool.EXPECT().RunWorkers(gomock.Any(), gomock.Any()).
		...
		Return(taskResultCh, nil)
	mockExecutionStorage := mock_action.NewMockScenarioExecutionStorage(ctrl)
	mockExecutionStorage.EXPECT().Inc(gomock.Any(), gomock.Any(), gomock.Any()).Times(0)
	mockExecutionStorage.EXPECT().Get(executionID).Return(&execution, nil).Times(2)
	mockExecutionStorage.EXPECT().Update(gomock.Any()).Return(nil)
	mockExecutionStorage.EXPECT().Del(executionID).Return(nil)
	mockScenarioStorage := mock_action.NewMockScenarioStorage(ctrl)
	logger := zerolog.Logger{}
