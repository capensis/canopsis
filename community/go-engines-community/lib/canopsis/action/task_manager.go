		...
		case scenariosTask, ok := <-inputChannel:
			...
			go func(ctx context.Context, task ExecuteScenariosTask) {
				if task.DelayedScenarioID != "" {
					...
					_, err := e.executionStorage.Inc(task.Alarm.ID, 1, true)
					if err != nil {
					...
					e.startExecution(*scenario, task.Alarm, task.Entity, task.AckResources)
					return
				}

				if task.AbandonedExecutionID != "" {
					execution, err := e.executionStorage.Get(task.AbandonedExecutionID)
					if err != nil {
...
func (e *redisBasedManager) finishExecution(
	...
	if execution.Tries > 0 {
		err := e.executionStorage.Del(execution.ID)
		if err != nil {
	...
	count, err := e.executionStorage.Inc(alarm.ID, -1, false)
	if err != nil {
...
func (e *redisBasedManager) processTaskResult(ctx context.Context, taskRes TaskResult) {
	scenarioExecution, err := e.executionStorage.Get(taskRes.ExecutionID)
	if err != nil || scenarioExecution == nil {
		e.logger.Err(err).Str("execution_id", taskRes.ExecutionID).Msg("cannot get execution")
		return
	}
	...
	err = e.executionStorage.Update(*scenarioExecution)
	if err != nil {
	...
	if scenarioExecution.ActionExecutions[taskRes.Step].Action.EmitTrigger &&
		...
		err := e.processEmittedTrigger(string(taskRes.AlarmChangeType), taskRes.Alarm,
...
func (e *redisBasedManager) processTriggers(task ExecuteScenariosTask) (bool, error) {
	err := e.scenarioStorage.RunDelayedScenarios(task.Triggers, task.Alarm, task.Entity)
	if err != nil {
		return false, err
	}
	...
	_, err = e.executionStorage.Inc(task.Alarm.ID, int64(len(scenarios)), true)
	if err != nil {
		return false, err
	}
...
func (e *redisBasedManager) processEmittedTrigger(
	...
	err := e.scenarioStorage.RunDelayedScenarios([]string{trigger}, alarm, entity)
	if err != nil {
		return err
	}
	...
	_, err = e.executionStorage.Inc(alarm.ID, int64(len(scenarios)), false)
	if err != nil {
		return err
	}

	for _, scenario := range scenarios {
		e.startExecution(scenario, alarm, entity, ackResource)
	}

	return nil
}

func (e *redisBasedManager) startExecution(scenario Scenario,
	...
	executionID, err := e.executionStorage.Create(ScenarioExecution{
