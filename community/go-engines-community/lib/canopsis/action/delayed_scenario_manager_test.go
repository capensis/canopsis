...
func TestDelayedScenarioManager_AddDelayedScenario_GivenNotDelayedScenario_ShouldReturnError(t *testing.T) {
	...
	err := manager.AddDelayedScenario(alarm, scenario)
	if err == nil {
...
func TestDelayedScenarioManager_AddDelayedScenario_GivenMatchedDelayedScenario_ShouldReturnTrue(t *testing.T) {
	...
	mockStorage.EXPECT().Add(gomock.Any()).Do(func(delayedScenario action.DelayedScenario) {
...
func TestDelayedScenarioManager_PauseDelayedScenarios_GivenNotPausedScenario_ShouldUpdateStorage(t *testing.T) {
	...
	mockStorage.EXPECT().GetAll().Return([]action.DelayedScenario{
		{
			ScenarioID:    "test-scenario-id",
			AlarmID:       "test-alarm-id",
			ExecutionTime: types.CpsTime{Time: time.Now().Add(10 * time.Second)},
			Paused:        false,
			TimeLeft:      0,
		},
		{
	...
	mockStorage.EXPECT().Update(gomock.Any()).Do(func(scenario action.DelayedScenario) {
		if !scenario.Paused {
	...
	err := manager.PauseDelayedScenarios(alarm)
	if err != nil {
		t.Errorf("expected not error but got %v", err)
	}
}
...
func TestDelayedScenarioManager_PauseDelayedScenarios_GivenPausedScenario_ShouldNotUpdateStorage(t *testing.T) {
	...
	mockStorage.EXPECT().Update(gomock.Any()).Times(0)
...
func TestDelayedScenarioManager_ResumeDelayedScenarios_GivenPausedScenario_ShouldUpdateStorage(t *testing.T) {
	...
	err := manager.ResumeDelayedScenarios(alarm)
	if err != nil {
		t.Errorf("expected not error but got %v", err)
	}
}
	...
	mockStorage.EXPECT().GetAll().Return([]action.DelayedScenario{
		{
			...
			ExecutionTime: types.CpsTime{Time: time.Now().Add(periodicalTimeout + time.Millisecond)},
		},
		{
			ID:            "test-delayed-id-2",
			ScenarioID:    expectedScenario.ID,
			AlarmID:       expectedAlarm.ID,
			ExecutionTime: types.CpsTime{Time: time.Now().Add(10 * periodicalTimeout)},
		},
	}, nil).Times(2)
	mockStorage.EXPECT().Delete(gomock.Eq("test-delayed-id")).Return(true, nil)
...
func TestDelayedScenarioManager_Run_GivenExpiredScenario_ShouldReturnItByWaitingGoroutine(t *testing.T) {
	...
	delayedScenario := action.DelayedScenario{
		...
		ScenarioID:    expectedScenario.ID,
		AlarmID:       expectedAlarm.ID,
		ExecutionTime: types.CpsTime{Time: time.Now().Add(3500 * time.Millisecond)},
	}
	mockStorage.EXPECT().GetAll().Return([]action.DelayedScenario{
		delayedScenario,
		{
	...
	mockStorage.EXPECT().Get(gomock.Eq(delayedScenario.ID)).Return(&delayedScenario, nil)
	mockStorage.EXPECT().Delete(gomock.Eq("test-delayed-id")).Return(true, nil)
	mockActionAdapter.EXPECT().GetEnabledByIDs(gomock.Eq([]string{expectedScenario.ID})).Return([]action.Scenario{expectedScenario}, nil)
