...
func (s *service) Process(event *types.Event) error {
	...
	case types.AlarmChangeTypePbhEnter, types.AlarmChangeTypePbhLeave,
		...
		if event.PbehaviorInfo.IsActive() {
			err = s.delayedScenarioManager.ResumeDelayedScenarios(alarm)
		} else {
			err = s.delayedScenarioManager.PauseDelayedScenarios(alarm)
		}
		if err != nil {
			return err
		}
...
func (s *service) ProcessAbandonedExecutions() error {
	abandonedExecutions, err := s.executionStorage.GetAbandoned()
	if err != nil {
		return err
	}
	...
	for _, execution := range abandonedExecutions {
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				...
				err = s.executionStorage.Del(execution.ID)
				if err != nil {
					return err
				}
			}
