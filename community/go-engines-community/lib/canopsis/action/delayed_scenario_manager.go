...
type DelayedScenarioManager interface {
	AddDelayedScenario(types.Alarm, Scenario) error
	PauseDelayedScenarios(types.Alarm) error
	ResumeDelayedScenarios(types.Alarm) error
	Run(ctx context.Context) (<-chan DelayedScenarioTask, error)
}
...
func (m *delayedScenarioManager) AddDelayedScenario(alarm types.Alarm, scenario Scenario) error {
	...
	id, err := m.storage.Add(delayedScenario)
	if err != nil {
		return err
	}
...
func (m *delayedScenarioManager) PauseDelayedScenarios(alarm types.Alarm) error {
	delayedScenarios, err := m.storage.GetAll()
	if err != nil {
		return err
	}
	...
	for _, delayedScenario := range delayedScenarios {
		if delayedScenario.AlarmID == alarm.ID && !delayedScenario.Paused {
			...
			_, err := m.storage.Update(delayedScenario)
			if err != nil {
				return err
			}
		}
...
func (m *delayedScenarioManager) ResumeDelayedScenarios(alarm types.Alarm) error {
	delayedScenarios, err := m.storage.GetAll()
	if err != nil {
		return err
	}
...
func (m *delayedScenarioManager) checkExpiredTimeoutScenario(ctx context.Context, ch chan<- DelayedScenarioTask) {
	expired, almostExpired, err := m.getDelayedScenarios()
	if err != nil {
...
func (m *delayedScenarioManager) waitAlmostExpiredTimeoutScenario(ctx context.Context, scenario DelayedScenario) {
	...
	case <-time.After(scenario.ExecutionTime.Sub(now)):
		updatedScenario, err := m.storage.Get(scenario.ID)
		if err != nil {
		...
		ok, err := m.storage.Delete(updatedScenario.ID)
		if err != nil {
...
func (m *delayedScenarioManager) getDelayedScenarios() (
	[]DelayedScenario,
	[]DelayedScenario,
	error,
) {
	...
	for _, delayedScenario := range delayedScenarios {
		...
		if now.Unix() >= delayedScenario.ExecutionTime.Unix() {
			ok, err := m.storage.Delete(delayedScenario.ID)
			if err != nil {
				return nil, nil, err
			}
