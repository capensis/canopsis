...
func (i *DedupList) SetBSON(raw bson.Raw) error {
	var list interface{}
	err := raw.Unmarshal(&list)
	if err != nil {
		return fmt.Errorf("DedupList.SetBSON unmarshal: %v", err)
	}

	i.reset()

	ilist, ok := list.([]interface{})
	if !ok {
		return fmt.Errorf("DedupList.SetBSON assertion: not []interface{}, got %T", list)
	}

	for idx, item := range ilist {
		sitem, ok := item.(string)
		if ok {
			i.Add(sitem)
		} else {
			return fmt.Errorf("DedupList.SetBSON Add: item %v:%v is not a string", idx, item)
		}
	}
