...
func TestGenDisplayName(t *testing.T) {
	Convey("Gen display name with 11 characters", t, func() {
		name := types.GenDisplayName("{{ rand_string 3 }}-{{ rand_string 3 }}-{{ rand_string 3 }}")
		So(name, ShouldHaveLength, 11)
		name1 := types.GenDisplayName("{{ rand_string 3 }}-{{ rand_string 3 }}-{{ rand_string 3 }}")
		So(name1, ShouldHaveLength, 11)
	})
}
