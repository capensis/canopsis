...
func NewAlarm(event Event, cfg config.CanopsisConf) (Alarm, error) {
...
func (a *Alarm) CurrentStatus(cfg config.CanopsisConf) CpsNumber {
	if a.Value.Status == nil {
		return a.ComputeStatus(cfg)
	}
...
func (a *Alarm) Update(e Event, cfg config.CanopsisConf) bool {
	timestamp := CpsTime(e.Timestamp)
	author := e.Connector + "." + e.ConnectorName
	message := e.Output

	updatedState := a.updateState(e)
	updatedStatus := a.UpdateStatus(timestamp, author, message, cfg)
...
func (a *Alarm) updateState(e Event) bool {
	if e.EventType != EventTypeCheck && e.EventType != EventTypeWatcher && e.EventType != EventTypeMetaAlarm {
		return false
	}
	...
	a.Value.LastUpdateDate = CpsTime(e.Timestamp)
...
// in parameters.
func (a *Alarm) UpdateStatus(timestamp CpsTime, author, message string, cfg config.CanopsisConf) bool {
	currentAlarmStatus := a.CurrentStatus(cfg)
...
func (a *Alarm) ComputeStatus(cfg config.CanopsisConf) CpsNumber {
	if a.IsCanceled() {
...
func (a Alarm) IsFlapping(cfg config.CanopsisConf) bool {
	lastStepType := ""
	freq := 0

	for i := len(a.Value.Steps) - 1; i >= 0; i-- {
		s := a.Value.Steps[i]
		duration := int(time.Since(s.Timestamp.Time).Seconds())
		if duration >= cfg.Alarm.FlappingInterval {
			break
		}
		...
		if freq > cfg.Alarm.FlappingFreqLimit {
			return true
		}
	}
...
func (a Alarm) IsStealthy(cfg config.CanopsisConf) bool {
	...
	for i := len(a.Value.Steps) - 1; i >= 0; i-- {
		...
		if int(timeSinceThisStep.Seconds()) >= cfg.Alarm.StealthyInterval {
			break
		}
