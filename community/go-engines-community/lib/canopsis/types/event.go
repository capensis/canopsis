...
func NewEventFromAlarm(alarm Alarm) Event {
	sourceType := SourceTypeComponent
	if alarm.Value.Resource != "" {
		sourceType = SourceTypeResource
	}
...
func (e Event) IsContextable() bool {
	switch e.EventType {
	case EventTypeCheck, EventTypePerf, EventTypeDeclareTicket, EventTypeUpdateWatcher, EventTypeMetaAlarm:
		return true
	default:
		return false
	}
...
func (e Event) IsValid() error {
	...
	switch e.SourceType {
	case SourceTypeComponent:
		break
	case SourceTypeMetaAlarm:
		break
	...
	case EventTypePerf:
		...
		// changestate event with zero state value is now valid
		//case EventTypeChangestate:
		//	fallthrough
		//case EventTypeKeepstate:
		//	if e.State == CpsNumber(0) {
		//		return errt.NewUnknownError(errors.New("cannot set state to info with changestate/keepstate"))
		//	}
	}
