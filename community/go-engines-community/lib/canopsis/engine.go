...
func (de *DefaultEngine) Started(runInfo engine.RunInfo) {
	de.Logger().Info().Msg("Engine started")
	de.saveRunInfo(runInfo)
}

func (de *DefaultEngine) saveRunInfo(runInfo engine.RunInfo) {
	if de.RunInfoManager != nil && runInfo.Name != "" {
		go func(info engine.RunInfo) {
			err := de.RunInfoManager.Save(info, de.Sleep)
			if err != nil {
...
func StartEngine(engine Engine, waitChan *chan os.Signal) (int, error) {
	...
	if engine.RunWorkerProcess() {
		...
		go routineWorker(engine, channel)
	} else {
		engine.AcknowledgeConfirmStop()
	}

	if engine.RunPeriodicalProcess() {
		go routinePeriodical(engine)
	} else {
		engine.AcknowledgeConfirmStop()
	}

	engine.Started(engine.GetRunInfo())
...
func (de *DefaultEngine) ProcessWorkerError(err error, msg amqp.Delivery) {
	...
	// Support old mongo package.
	if _, ok := err.(mongo.MongoConnectionError); ok {
		de.NackMessage(msg)
		de.AskStop(ExitEngine)
		return
	}
...
func routineWorker(engine Engine, channel <-chan amqp.Delivery) {
...
func routinePeriodical(engine Engine) {
