...
func (a *adapter) loadRoles(ctx context.Context, model model.Model) error {
	...
	for _, r := range roles {
		for objId, obj := range r.Permissions {
			if obj.Type != "" {
				if permBitmasksByName, ok := permBitmasksByType[obj.Type]; ok {
					for permName, bitmask := range permBitmasksByName {
						if obj.Bitmask&bitmask == bitmask {
							err = model.AddPolicy(sec, ptype, []string{r.ID, objId, permName})
							if err != nil {
								return err
							}
						}
				...
				err = model.AddPolicy(sec, ptype, []string{r.ID, objId, libmodel.PermissionCan})
				if err != nil {
					return err
				}
			}
...
func (a *adapter) loadSubjects(ctx context.Context, model model.Model) (resErr error) {
	...
	for cursor.Next(ctx) {
		...
		for _, r := range u.Roles {
			err = model.AddPolicy(sec, ptype, []string{u.ID, r})
			if err != nil {
				return err
			}
		}
...
func (a *adapter) getRolesWithPermissions(ctx context.Context) ([]role, error) {
	...
	err = cursor.All(ctx, &permissionDocs)
	if err != nil {
		return nil, err
	}
	...
	for _, permission := range permissionDocs {
