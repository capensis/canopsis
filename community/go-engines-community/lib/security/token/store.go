...
type Token struct {
	...
	Accessed libtime.CpsTime `bson:"accessed"`

	Expired             *libtime.CpsTime          `bson:"expired,omitempty"`
	ExpiredByInactivity *libtime.CpsTime          `bson:"expired_by_inactivity,omitempty"`
	MaxInactiveInterval *libtime.DurationWithUnit `bson:"max_inactive_interval,omitempty"`
}
...
func (s *MongoStore) Count(ctx context.Context) (int64, error) {
	return s.collection.CountDocuments(ctx, bson.M{
		"$or": []bson.M{
			{"expired": nil},
			{"expired": bson.M{"$gt": libtime.NewCpsTime()}},
		},
	})
}
