...
func TestMessageProcessor_Process_GivenRecomputeEntityServiceEvent_ShouldReplaceItToUpdateEntityService(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	body := []byte("{\"event_type\":\"recomputeentityservice\"}")
	event := types.Event{
	...
	mockEncoder.EXPECT().Encode(gomock.Any()).Do(func(event types.Event) {
		if event.EventType != types.EventTypeUpdateEntityService {
			t.Errorf("expected event %s but got %s", types.EventTypeUpdateEntityService, event.EventType)
		}
	}).Return(expectedBody, nil)
	processor := &messageProcessor{
		FeatureEventProcessing: true,
