...
func NewFailoverOptions(sURL string, db int, logger zerolog.Logger,
	...
	failoverOptions.OnConnect = func(cn *redis.Conn) error {
...
func NewSession(db int, logger zerolog.Logger, reconnectCount int, minReconnectTimeout time.Duration) (*redis.Client, error) {
