...
func TestBadRedisOptions(t *testing.T) {
	Convey("Testing bad redis urls", t, func() {
		...
		Convey("Bad url - i'm calling you", func() {
			...
			_, err := redis.NewSession(-1, log.NewTestLogger(), 0, 0)
...
func TestNewRedisSession(t *testing.T) {
	_, err := redis.NewSession(redis.CacheAlarm, log.NewTestLogger(), 0, 0)
	if err != nil {
...
func BenchmarkRedisSpeed(b *testing.B) {
	client, _ := redis.NewSession(redis.CacheAlarm, log.NewTestLogger(), 0, 0)

	for i := 1; i < b.N; i++ {
		rid := "titi_" + strconv.Itoa(i)
		if i%4 == 0 {
			err := client.Set(rid, "toto", time.Hour*4)
			if err.Err() != nil {
