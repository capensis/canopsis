...
// NewSession creates a new connection to the MongoDB database.
// It uses the EnvCpsMongoUrl as configuration.
// Use NewClient to migrate to mongo-driver package.
func NewSession(timeout time.Duration) (*mgo.Session, error) {
	...
	// Wait longer on regular actions.
	session.SetSocketTimeout(TimeoutWork)
	session.SetSyncTimeout(TimeoutWork)
...
func IsConnectionError(err error) bool {
	...
	if commErr, ok := err.(mongo.CommandError); ok {
		for _, label := range commErr.Labels {
			if label == driver.NetworkError {
				return true
			}
		}
	}
