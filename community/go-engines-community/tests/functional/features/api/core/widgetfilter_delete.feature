Feature: Delete a widget filter
  ...
  Scenario: given personal delete request should delete filter
  ...
  Scenario: given delete request and auth user without view permission should not allow access
    When I am admin
    When I do DELETE /api/v4/widget-filters/test-widgetfilter-to-check-access
    Then the response code should be 403

  Scenario: given delete request with not exist id should return not allow access error
    When I am admin
    When I do DELETE /api/v4/widget-filters/test-widgetfilter-not-found
    Then the response code should be 403

  Scenario: given delete request and someone else filter should return error
    When I am admin
