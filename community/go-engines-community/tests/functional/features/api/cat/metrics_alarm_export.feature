Feature: Export alarm metrics
  ...
  Scenario: given export request should return metrics
    When I am admin
    When I do POST /api/v4/cat/metrics-export/alarm?parameters[]=created_alarms&sampling=day&from={{ parseTime "20-11-2021 00:00" }}&to={{ parseTime "24-11-2021 00:00" }}&kpi_filter=test-kpi-filter-to-alarm-metrics-get
  ...
  Scenario: given export request with empty interval should return metrics with zeros
    When I am admin
    When I do POST /api/v4/cat/metrics-export/alarm?parameters[]=created_alarms&sampling=day&from={{ parseTime "06-09-2020 00:00" }}&to={{ parseTime "08-09-2020 00:00" }}&kpi_filter=test-kpi-filter-to-alarm-metrics-get
    Then the response code should be 200
    When I save response exportID={{ .lastResponse._id }}
    When I do GET /api/v4/cat/metrics-export/{{ .exportID }} until response code is 200 and body contains:
    """json
  ...
  Scenario: given export request with filter by entity infos should return metrics
    When I am admin
    When I do POST /api/v4/cat/metrics-export/alarm?parameters[]=created_alarms&sampling=day&from={{ parseTime "20-11-2021 00:00" }}&to={{ parseTime "24-11-2021 00:00" }}&kpi_filter=test-kpi-filter-to-alarm-metrics-get-by-entity-infos
    Then the response code should be 200
    When I save response exportID={{ .lastResponse._id }}
    When I do GET /api/v4/cat/metrics-export/{{ .exportID }} until response code is 200 and body contains:
    """json
  ...
  Scenario: given export request with invalid query params should return bad request
    ...
    """
    When I do POST /api/v4/cat/metrics-export/alarm?kpi_filter=not-exist&from={{ now }}&to={{ now }}&sampling=day&parameters[]=created_alarms
    Then the response code should be 400
    Then the response body should be:
    """json
    {
