Feature: Metrics should be added on alarm changes
  ...
  Scenario: given alarm should add not_acked_alarms metric
    ...
    """
    When I do GET /api/v4/cat/metrics/alarm?filter={{ .filterID }}&parameters[]=not_acked_alarms&parameters[]=not_acked_in_hour_alarms&parameters[]=not_acked_in_four_hours_alarms&parameters[]=not_acked_in_day_alarms&sampling=day&from={{ nowDateTz }}&to={{ nowDateTz }} until response code is 200 and body contains:
    """json
    {
      "data": [
        {
  ...
  Scenario: given alarm should add not_acked_in_hour_alarms metric
    ...
    Then the response code should be 201
    When I save response filterID={{ .lastResponse._id }}
    When I do GET /api/v4/cat/metrics/alarm?filter={{ .filterID }}&parameters[]=not_acked_in_hour_alarms&parameters[]=not_acked_in_four_hours_alarms&parameters[]=not_acked_in_day_alarms&sampling=day&from={{ nowDateTz }}&to={{ nowDateTz }} until response code is 200 and body contains:
    """json
    {
      "data": [
        {
  ...
  Scenario: given alarm pbhenter with active type should not remove not_acked_in_hour_alarms metric
    ...
    When I do GET /api/v4/cat/metrics/alarm?filter={{ .filterID }}&parameters[]=not_acked_in_hour_alarms&parameters[]=not_acked_in_four_hours_alarms&parameters[]=not_acked_in_day_alarms&sampling=day&from={{ nowDateTz }}&to={{ nowDateTz }}
