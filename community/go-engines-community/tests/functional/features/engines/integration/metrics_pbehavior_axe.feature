Feature: SLI metrics should be added on alarm changes
  ...
  Scenario: given entity in maintenance pbehavior and new alarm should add SLI maintenance metrics
    ...
    """
    When I do GET /api/v4/cat/metrics/sli?filter={{ .filterID }}&sampling=day&from={{ nowDateTz }}&to={{ nowDateTz }} until response code is 200 and response key "data.0.maintenance" is greater or equal than 4
    Then the response body should contain:
    """json
    {
      "data": [
