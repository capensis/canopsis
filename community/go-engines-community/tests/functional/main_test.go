...
func InitializeScenario(
	...
	return func(scenarioCtx *godog.ScenarioContext) {
		...
		scenarioCtx.Step(`^I add form field ([\w]+)=(.+)$`, apiClient.IAddFormField)
		scenarioCtx.Step(`^I add form file ([\w]+)=(.+)$`, apiClient.IAddFormFile)
		...
		scenarioCtx.Step(`^the response body should contain file ([^\s]+)$`, apiClient.TheResponseBodyShouldContainFile)
