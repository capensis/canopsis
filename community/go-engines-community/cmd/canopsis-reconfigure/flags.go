...
func (f *flags) Parse() {
	...
	flag.BoolVar(&f.modeMigrate, "migrate", false, "If true, it will execute migration scripts")
	flag.BoolVar(&f.modeMigrateOnly, "migrate-only", false, "If true, it will only execute migration scripts")
	flag.StringVar(&f.migrationDirectory, "migration-directory", "", "The directory with migration scripts")
