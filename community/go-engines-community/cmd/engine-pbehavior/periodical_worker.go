...
func (w *periodicalWorker) Work() error {
	...
	_, err := w.LockerClient.Obtain(redis.PeriodicalLockKey, w.GetInterval(), nil)
	...
	computeLock, err := w.LockerClient.Obtain(redis.RecomputeLockKey, redis.RecomputeLockDuration, &redislock.Options{
	...
	ok, err := w.Store.Restore(w.PbhService)
	if err != nil {
	...
	cursor, err := alarmCollection.Aggregate(ctx, []bson.M{
		...
		{"$lookup": bson.M{
			"from":         libentity.EntityCollectionName,
			"localField":   "alarm.d",
			"foreignField": "_id",
			"as":           "entity",
		}},
