...
func (m DependencyMaker) getRedisLockerClient(logger zerolog.Logger, cfg config.CanopsisConf) redis.LockClient {
	return redis.NewLockClient(m.DepRedisSession(redis.PBehaviorLockStorage, logger, cfg))
}

func (m DependencyMaker) getRedisStore(logger zerolog.Logger, cfg config.CanopsisConf) redis.Store {
	store := redis.NewStore(m.DepRedisSession(redis.PBehaviorLockStorage, logger, cfg), "pbehaviors", 0)

	return store
}

func NewEnginePBehavior(options Options, logger zerolog.Logger) engine.Engine {
	m := DependencyMaker{}
	cfg := m.DepConfig()
	...
	store := m.getRedisStore(logger, cfg)
	...
	enginePbehavior := engine.New(
		func(ctx context.Context) error {
			...
			computeLock, err := lockerClient.Obtain(redis.RecomputeLockKey, redis.RecomputeLockDuration, &redislock.Options{
			...
			ok, err := store.Restore(pbhService)
			if err != nil {
