...
func (e *EngineFIFO) Initialize() error {
	e.References.Scheduler.Start()
...
func (e *EngineFIFO) PeriodicalProcess() {
	select {
	case <-e.ctlChan:
...
func (e *EngineFIFO) WorkerProcess(msg amqp.Delivery) {
	ctx, task := trace.NewTask(context.Background(), "fifo.WorkerProcess")
	defer task.End()
	...
	err = e.References.Scheduler.ProcessEvent(e.References.ChannelPub, event.GetLockID(), msg.Body)
...
func (e *EngineFIFO) ackManager() {
	...
	for msg := range ackChan {
		...
		if err := e.References.Scheduler.AckEvent(e.References.AckChanPub, event); err != nil {
			e.Logger().Err(err).
				Str("lockID", event.GetLockID()).
