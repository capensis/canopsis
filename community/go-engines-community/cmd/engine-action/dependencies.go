...
func NewEngineAction(ctx context.Context, options Options, logger zerolog.Logger) engine.Engine {
	...
	actionService := action.NewService(alarmAdapter, scenarioExecChan,
		...
		alarm.NewActivationService(json.NewEncoder(), amqpChannel, canopsis.CheQueueName),
		logger)
	...
	engineAction := engine.New(
		func(ctx context.Context) error {
			...
			manager := action.NewTaskManager(
				action.NewWorkerPool(options.WorkerPoolSize, mongoClient, axeRpcClient, webhookRpcClient, json.NewEncoder(), logger, templateExecutor, alarmConfigProvider),
				storage,
