...
func testNewEngineHeartBeat(ctx context.Context) (*TestEngineHeartBeat, *testSendAlarm, error) {
	tsa := testSendAlarm{
		Alarms: make([]string, 0),
	}

	depMaker := hb.DependencyMaker{}
	references := depMaker.GetDefaultReferences(ctx, log.NewTestLogger())
...
func TestInitializeHeartBeat(t *testing.T) {
	...
	Convey("Setup", t, func() {
		...
		engine, _, err := testNewEngineHeartBeat(ctx)
		...
		Convey("Initialize will set correct warmups", func() {
			...
			r := engine.References.Redis.Exists(ctx, "connector:c1")
			So(r.Err(), ShouldBeNil)
			So(r.Val(), ShouldEqual, 1)

			rttl := engine.References.Redis.TTL(ctx, "connector:c1")
...
func TestWorkHeartBeat(t *testing.T) {
	...
	Convey("Setup", t, func() {
		...
		Convey("Given an event matching LI1, i must set an ID in Redis", func() {
			engine.References.Redis.FlushDB(ctx)
			...
			Convey("I have only li2.ID() set", func() {
				r := engine.References.Redis.Exists(ctx, li2.ID())
				So(r.Err(), ShouldBeNil)
				So(r.Val(), ShouldEqual, 1)
				rttl := engine.References.Redis.TTL(ctx, li2.ID())
				So(rttl.Err(), ShouldBeNil)
				So(rttl.Val(), ShouldBeBetweenOrEqual, li1.MaxDuration-time.Second-1, li1.MaxDuration)
				r = engine.References.Redis.Exists(ctx, li1.ID())
				So(r.Err(), ShouldBeNil)
				So(r.Val(), ShouldEqual, 0)
				r = engine.References.Redis.Exists(ctx, li3.ID())
				So(r.Err(), ShouldBeNil)
				So(r.Val(), ShouldEqual, 0)
			})
		})
...
func TestHeartBeatPeriodicalProcess(t *testing.T) {
	...
	Convey("Setup", t, func() {
		engine, tsa, err := testNewEngineHeartBeat(ctx)
		So(err, ShouldBeNil)
		So(tsa, ShouldNotBeNil)
		engine.References.Redis.FlushDB(ctx)

		status := engine.References.Redis.Set(ctx, "connector:testc.connector_name:testcn", 1, time.Millisecond*500)
		...
		Convey("When i run PeriodicalProcess a first time", func() {
			...
			Convey("When i send an event matching first alarm", func() {
				...
				r := engine.References.Redis.Exists(ctx, "alarm:connector:testc.connector_name:testcn")
				So(r.Err(), ShouldBeNil)
				So(r.Val(), ShouldEqual, 0)

				engine.WorkerProcess(ctx, evt)
				So(len(tsa.Alarms), ShouldEqual, 2)
				So(tsa.Alarms[0], ShouldEqual, "connector:testc.connector_name:testcn")
				So(tsa.Status[0], ShouldEqual, 0)
				So(tsa.Status[1], ShouldEqual, 3)

				r = engine.References.Redis.Exists(ctx, "alarm:" + tsa.Alarms[0])
				So(r.Err(), ShouldBeNil)
				So(r.Val(), ShouldEqual, 0)
			})
		})
...
func BenchmarkWorkHeartBeat(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		engine.WorkerProcess(ctx, amqp.Delivery{
			Body: []byte(sevent),
		})
	}
}
