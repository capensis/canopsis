...
func (e *EngineHeartBeat) WorkerProcess(parentCtx context.Context, msg amqp.Delivery) {
	ctx, task := trace.NewTask(parentCtx, "heartbeat.WorkerProcess")
	defer task.End()
	...
	_, err = e.workHeartBeat(ctx, ievent)
	if err != nil {
		e.processWorkerError(err, msg)
		return
	}
...
func (e *EngineHeartBeat) PeriodicalProcess(ctx context.Context) {
	...
	for _, heartBeatItem := range e.HeartBeatItems.Value() {
		...
		result := e.References.Redis.Exists(ctx, id)
		err = result.Err()
		if result.Val() == 0 {
			err := e.SendAlarmFunc(id, types.AlarmStateCritical, heartBeatItem.Output)
			if err == nil {
				e.References.Redis.Set(ctx, "alarm:"+id, 0, 0)
			}
		} else {
...
func (e *EngineHeartBeat) warmup(ctx context.Context, li heartbeat.Item) error {
	res := e.References.Redis.Exists(ctx, li.ID())
	if res.Err() == nil && res.Val() == 0 {
		setr := e.References.Redis.Set(ctx, li.ID(), 0, li.MaxDuration)
		return setr.Err()
	}
...
func (e *EngineHeartBeat) Initialize(ctx context.Context) error {
	if err := e.DefaultEngine.Initialize(ctx); err != nil {
		return err
	}
...
func (e *EngineHeartBeat) workHeartBeat(ctx context.Context, ievent types.GenericEvent) ([]int, error) {
	...
	for itemCount, lm := range e.HeartBeatItems.Value() {
		...
		// exists or no.
		cmdstatus := e.References.Redis.Set(ctx, id, nil, lm.MaxDuration)
		...
		alarmID := "alarm:" + id
		cmdint := e.References.Redis.Exists(ctx, alarmID)
		if cmdint.Err() != nil {
		...
		if cmdint.Val() != 0 {
			...
			cmddel := e.References.Redis.Del(ctx, alarmID)
			if cmddel.Err() != nil {
