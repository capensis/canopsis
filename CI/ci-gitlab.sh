...
docker-compose -f docker-compose.ci.yml -p ${COMPOSE_PROJECT_NAME} up -d webserver mongodb redis rabbitmq influxdb provisionning
./docker/wait-provisionning.sh CI/docker-compose.ci.yml ${COMPOSE_PROJECT_NAME}
docker-compose -f docker-compose.ci.yml -p ${COMPOSE_PROJECT_NAME} restart webserver mongodb redis rabbitmq influxdb provisionning
