...
manager = manager(logging_level=INFO)

base_component_event = forger(
    connector='bench',
...
def send_events(n, rate=0, burst=10):
    ...
    if (rate):
        #1/10 ou 1/15
        logger.info(
            " + @ %s events/second (%s events/5min)" % (rate, (rate * 300)))
        #time_target = time() + float(n)/rate
    ...
    while RUN and i < n:
        ...
        if (rate and (i % burst == 0)):
            elapsed = time() - time_start_burst
            if (time_break > elapsed):
                sleep(time_break - elapsed)
            time_start_burst = time()
    ...
    logger.info(
        " + Done, elapsed: %.3f ms (%s events/second)" % (
    ...
    timeout = time() + 300
    while RUN:
        raw = storage.find_one(
            {'_id': rk, 'benchId': benchId}, mfields={'bench_timestamp': 1})
        if raw:
            elapsed = time() - float(raw['bench_timestamp'])
...
def clean_db():
    ...
    for perf_data in base_component_event['perf_data_array']:
        manager.remove(
            name="%sbench%s" % (
                base_component_event['component'], perf_data['metric']))
