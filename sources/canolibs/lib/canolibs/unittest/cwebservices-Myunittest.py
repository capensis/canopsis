...
class KnownValues(unittest.TestCase): 
	...
	def test_05_Putting_View(self):
		WS.login('root', 'root')
		output = WS.put_view(self.view_id,self.view_name,self.parent_directory)
		try:
	...
	def test_06_Putting_Directory(self):
		WS.login('root', 'root')
		output = WS.put_view(self.directory_id,self.directory_name,self.parent_directory,leaf=False)
		try:
			output = json.loads(output)
		except:
			raise Exception('Invalid webserver output')
