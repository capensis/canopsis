...
class engine(Engine):
    ...
    def pre_run(self):
        ...
        self.internal_amqp = Amqp(logging_level=self.logging_level, logging_name='{0}-internal-amqp'.format(self.name))
    ...
    def to_perfstore(self, rk, perf_data, timestamp, component, resource=None, tags=None):
        ...
        if isinstance(perf_data, list):
            ...
            for perf in perf_data:
                ...
                self.logger.debug(
                    " + Put metric '%s' (%s %s (%s)) for ts %s ..." % (
                        metric, value, unit, dtype, timestamp))

                if value is None:
                    self.logger.warning(
                        "Invalid value: '%s' (%s: %s)" % (value, rk, metric))
                    continue
                ...
                try:
                    ...
                    self.logger.warning(
                        'Impossible to put value in perfstore (%s) (metric=%s, unit=%s, value=%s)' % (err, metric, unit, value))

        else:
            raise Exception(
                "Imposible to parse: %s (is not a list)" % perf_data)
    ...
    def work(self, event, *args, **kargs):
        ...
        if perf_data:
            ...
            try:
                ...
                self.logger.error(
                    "Impossible to parse: %s ('%s')" % (perf_data, err))
    ...
    def on_internal_event(self, event, msg):
        ...
        if perf_data_array:
            ...
            try:
                ...
                self.logger.warning(
                    "Impossible to store: %s ('%s')" % (perf_data_array, err))
    ...
    def beat(self):
        ...
        count = self.storage.get_backend('perfdata2').find(
            {'me': {'$nin': internal_metrics}}).count()
        ...
        self.logger.warning(
            'Cache value for perfdata2 metric count computed > %s' + count)
