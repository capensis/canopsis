...
class engine(TaskHandler):
	...
	def handle_task(self, job):
		...
		raccount = caccount(user='root', group='root', mail='root@localhost.local')
		rstorage = get_storage('object', account=raccount)

		taccount = rstorage.find_one(mfilter = {
			'_id': 'account.{0}'.format(account)
		})

		if isinstance(taccount, crecord):
			account = caccount(taccount)
		...
		if owner:
			towner = rstorage.find_one(mfilter = {
				'_id': 'account.{0}'.format(owner)
			})

			if isinstance(towner, crecord):
				owner = caccount(towner)
		...
		try:
			...
			return (
				2,
				"Can't find view {0} with account {1}".format(viewname, account.user)
			)

		self.logger.info('Account {0} requested rendering the view {1}'.format(account.user, view.name))
