...
from canopsis.engine import Engine, DROP
...
class engine(Engine):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.storage = get_storage(
            logging_level=self.logging_level, account=account)
    ...
    def beat(self, *args, **kargs):
        ...
        self.configuration = {
            'rules' : [], 'default_action': self.find_default_action()}
        try:
            records = self.storage.find(
                {'crecord_type': 'rule'}, sort='priority')
    ...
    def send_stat_event(self):
        ...
        self.logger.debug(
            "%s event dropped since %s" % (
                self.drop_event_count, self.beat_interval))
        self.logger.debug(
            "%s event passed since %s" % (
