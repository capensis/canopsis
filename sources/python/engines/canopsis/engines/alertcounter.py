...
class engine(Engine):
    ...
    def load_macro(self):
        ...
        record = self.storage.get_backend('object').find_one(
    ...
    def reload_ack_comments(self):
        ...
        query = self.objects_backend.find(
            {'crecord_type': 'comment', 'referer_event_rks': {'$exists': True}},
            {'referer_event_rks': 1})
    ...
    def increment_counter(self, meta, value):
        ...
        self.manager.put_data(
            metric_id=metric_id,
            meta=meta,
            timestamp_with_values=((time(), value)))
    ...
    def resolve_selectors_name(self):
        ...
        if int(time()) > (self.last_resolv + 60):
