...
from canopsis.context import Context
from canopsis.dependency import Dependency
...
class engine(Engine):
    ...
    def topo_getOperator_fn(self, name):
        ...
        if not module:
            try:
                ...
                self.logger.warning(
                    "Impossible to load operator '%s' (%s)" % (name, err))
    ...
    def topo_getState(self, topo):
        ...
        def parseChilds(parent, level=0):
            ...
            self.logger.debug(
                '' * level + "|-> %s (State: %s, type: %s)" % (
                    _id, state.get('state', None),
                    state.get('state_type', None)))
            ...
            if childIds:
                ...
                # Set state
                self.stateById[_id] = {
                    'state': state, 'state_type': 1, 'previous_state': state}
    ...
    def topo_dump4Ui(self, topo):
        ...
        def parseChilds(parent):
            ...
            states = self.stateById.get(
                parent['_id'],
                {'state': 3, 'state_type': 1, 'previous_state': 3})
        ...
        if topology:
            ...
            self.logger.debug(
                "Parse topo '%s': %s Nodes with %s Conns" % (
                    topo['crecord_name'], len(topo['nodes']),
                    len(topo['conns'])))
            ...
            self.logger.debug(
                "'%s': State: %s" % (topo['crecord_name'], states_info))
    ...
    def work(self, event, *args, **kargs):
        ...
        # process the event if type a component, resource or topology-nodeis among those ones
        if entity_type in ('component', 'resource'):
            pass

        # process the event 
        elif entity_type == 'topology-node':
            pass
