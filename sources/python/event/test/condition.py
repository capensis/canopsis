...
from canopsis.rule.condition import check
...
class KnownValues(TestCase):
    ...
    def test_06_complex(self):
        ...
        filter1 = {'$and': [
            {'timestamp': {'$gt': 0}}, {'timestamp': {'$lt': 2378713357}}]}
        match = check(filter1, event)
        self.assertTrue(match, msg='Filter: %s' % filter1)

        filter1 = {'connector': {'$eq': 'Engine'},
            'timestamp': {'$gt': 137871335}}
        match = check(filter1, event)
        self.assertTrue(match, msg='Filter: %s' % filter1)
        ...
        filter1 = {'$nor': [
            {'connector': {'$eq': 'cEngine'}},
            {'connector': {'$eq': 'ccEngine'}}]}
        match = check(filter1, event)
        self.assertTrue(match, msg='Filter: %s' % filter1)
        ...
        filter1 = {'$and': [
            {'connector': 'Engine'}, {'event_type': 'check'},
            {'event_type': 'check'}]}
        match = check(filter1, event)
        self.assertTrue(match, msg='Filter: %s' % filter1)

        filter1 = {'$or': [
            {'connector': 'cenginddddde'}, {'event_type': 'check'},
            {'event_type': 'checkkkkk'}]}
        match = check(filter1, event)
        self.assertTrue(match, msg='Filter: %s' % filter1)

        filter1 = {'$or': [{'$and': [{'connector': 'cenginddddde'},
            {'event_type': 'check'}]}, {'event_type': 'checkkkkk'}]}
        match = check(filter1, event)
        self.assertFalse(match, msg='Filter: %s' % filter1)
