	...
	def load_crecords(self):
		...
		if lock:
			...
			if lock['lock'] and now - lock['last_update'] < UNLOCK_DELAY:
				self.logger.debug('Information beeing threan by another dispatcher engine. Nothing has to be done. delay : {0}'.format(now - lock['last_update']))
				return

			elif now - lock['last_update'] > UNLOCK_DELAY:
				#Information message
				self.logger.debug('Dispatcher lock duration exeeded lock limit ({0} seconds) , record selection will be performed'.format(UNLOCK_DELAY))
