...
from cstorage.one_value import OneValueStorage
...
class OneValueStorage(Storage, OneValueStorage):
	"""
	...
	def _get_indexes(self):
		...
		result = super(OneValueStorage, self)._get_indexes()

		result.append(OneValueStorage.Index.QUERY)
	...
	def size(self, data_id=None, aggregation=None, period=None, timewindow=None):
		...
		where = {
			OneValueStorage.Index.DATA_ID: data_id
		}

		if aggregation is not None:
			where[OneValueStorage.Index.AGGREGATION] = aggregation

		if timewindow is not None:
			where[OneValueStorage.Index.TIMESTAMP] = {
				'$gte': timewindow.start(),
				'$lte': timewindow.stop()
			}

		if period is not None:
			where[OneValueStorage.Index.PERIOD] = period

		cursor = self._find(document=where)
		cursor.hint(OneValueStorage.Index.QUERY)
	...
	def get(self, data_id, aggregation, period, timewindow=None, limit=0):
		...
		projection = {
			OneValueStorage.Index.TIMESTAMP: 1,
			OneValueStorage.Index.VALUES: 1
		}

		if aggregation is None:
			projection[OneValueStorage.Index.AGGREGATION] = 1

		if period is None:
			projection[OneValueStorage.Index.PERIOD] = 1
		...
		for document in cursor:
			...
			timestamp = int(document[OneValueStorage.Index.TIMESTAMP])

			values = document[OneValueStorage.Index.VALUES]
	...
	def put(self, data_id, aggregation, period, points):
		...
		for timestamp, value in points:
			...
			if '_id' not in document_properties:
				document_properties['_id'] = OneValueStorage._get_document_id(
					data_id=data_id, aggregation=aggregation,
					timestamp=id_timestamp, period=period)
				document_properties[OneValueStorage.Index.LAST_UPDATE] = timestamp

			else:
				if document_properties[OneValueStorage.Index.LAST_UPDATE] < timestamp:
					document_properties[OneValueStorage.Index.LAST_UPDATE] = timestamp

			field_name = "{0}.{1}".format(
				OneValueStorage.Index.VALUES, timestamp - id_timestamp)
		...
		for id_timestamp, document_properties in \
			...
			_set = {
				OneValueStorage.Index.DATA_ID: data_id,
				OneValueStorage.Index.AGGREGATION: aggregation,
				OneValueStorage.Index.PERIOD: period.unit_values,
				OneValueStorage.Index.TIMESTAMP: id_timestamp
			}
	...
	def remove(self, data_id, aggregation=None, period=None, timewindow=None):
		...
		if timewindow is not None:
			...
			for document in documents:
				timestamp = document.get(OneValueStorage.Index.TIMESTAMP)
				values = document.get(OneValueStorage.Index.VALUES)
				...
				if len(values_to_save) > 0:
					self._update(
						_id={'_id': _id},
						document={
							'$set': {OneValueStorage.Index.VALUES: values_to_save}
						})
				else:
	...
	def _get_documents_query(self, data_id, aggregation, timewindow, period):
		...
		if aggregation is not None:
			result[OneValueStorage.Index.AGGREGATION] = aggregation

		if period is not None:  # manage specific period
			result[OneValueStorage.Index.PERIOD] = period.unit_values

		if timewindow is not None:  # manage specific timewindow
			start_timestamp, stop_timestamp = \
				OneValueStorage._get_id_timestamps(
					timewindow=timewindow, period=period)
			result[OneValueStorage.Index.TIMESTAMP] = {
				'$gte': start_timestamp,
				'$lte': stop_timestamp}
	...
	def _get_document_id(data_id, aggregation, timestamp, period):
		...
		if unit_with_value is None:
			raise OneValueStorage.OveValueStorageError(
