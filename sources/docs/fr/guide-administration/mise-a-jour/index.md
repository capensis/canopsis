...
!!! note
    Dans la plupart des cas, la mise à jour d'un environnement installé via docker consistera à mettre à jour les versions de tag des images docker utilisées dans la stack. Dans tous les cas, il est recommandé d'aller lire les [notes de version](https://doc.canopsis.net/#dernieres-versions) entre la version à mettre à jour et la version qui doit être installée.
