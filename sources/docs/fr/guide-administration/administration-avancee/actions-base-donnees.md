...
!!! attention
    Cette manipulation a un impact métier important et ne doit être réalisée que par une personne compétente. **Avant toute opération, il est vivement conseillé de faire une [sauvegarde de la base MongoDB](#sauvegarde)** en utilisant `mongorestore` **ainsi que d'arrêter Redis** (`systemctl stop redis`) **et le moteur `che`.**
