...
La documentation se trouve ici ([Widget Compteur](https://doc.canopsis.net/guide-utilisation/interface/widgets/compteur/))
...
*  **Connecteurs :**
    *  **email2canopsis :**
        ...
        *  Le sélectionneur `line` prend maintenant en charge le multi-ligne ([#2242](https://git.canopsis.net/canopsis/canopsis/issues/2242))
        ...
        *  Possibilité dans le fichier `ini` de spécifier le sender à l'aide d'une expression régulière ([#2239](https://git.canopsis.net/canopsis/canopsis/issues/2239))
...
*  **Moteurs :**
    *  **Moteurs nouvelle génération (Go) :**
        *  Correction d'un bug entrainant un plantage et redémarrage du moteur ̀ watcher` ([#2254](https://git.canopsis.net/canopsis/canopsis/issues/2254))
...
*  **UI :**
    *  **Bac à alarmes :**
        *  Prise en charge des types `pbehaviors` dans la Time line ([#2290](https://git.canopsis.net/canopsis/canopsis/issues/2290))
        ...
        *  L'accès à la liste des variables brutes est désormais liée à un droit (et non plus liée à l'édition du widget) ([#2235](https://git.canopsis.net/canopsis/canopsis/issues/2235))
...
* **Expérience utilisateur :**
    ...
    *  Le bouton permettant de raffraichir une vue est désormais positionné horizontalement ([#2212](https://git.canopsis.net/canopsis/canopsis/issues/2212))
