...
Le moteur `task_mail` est maintenant considéré comme déprécié et n'est plus activé dans les nouvelles installations. Il est plutôt recommandé d'utiliser le [mécanisme des Webhooks](../guide-administration/moteurs/moteur-axe-webhooks.md) vers un service d'envoi d'emails.
