...
En environnement Go, le fichier `default_configuration.toml` comporte quelques variables permettant de configurer des comportements génériques des moteurs (cf. [documentation du moteur `engine-axe`](../guide-administration/moteurs/moteur-axe.md)).
...
Canopsis 3.37.0 ajoute un nouvelle option de lancement `-ignoreDefaultTomlConfig` au moteur `engine-axe`. Lorsque le moteur est appelé avec cette option (désactivée par défaut), le fichier de configuration sera resynchronisé en base à chaque redémarrage.
...
*  **Moteurs :**
    *  **Moteurs nouvelle génération (Go) :**
        ...
        *  Ajout d'une nouvelle option de lancement `-ignoreDefaultTomlConfig` au moteur `engine-axe`, permettant de forcer la resynchronisation du fichier de configuration `default_configuration.toml` en base (voir plus haut) ([#179](https://git.canopsis.net/canopsis/go-engines/issues/179))
