...
Le *linkbuilder* entre en jeu au sein du `webserver` de Canopsis, c'est-à-dire
un composant de l'environnement Python de Canopsis.
...
fichier de log au chemin renseigné, sur la machine où le `webserver` de Canopsis
est exécuté.

## Mettre en service le linkbuilder

Placer le fichier Python dans le dossier `common/link_builder/` des sources de
Canopsis. Redémarrer le service `webserver`.
