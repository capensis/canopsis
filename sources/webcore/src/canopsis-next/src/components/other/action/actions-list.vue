    ...
    div.white
      v-layout(row, wrap)
        ...
        v-flex(v-show="selected.length", xs4)
          v-btn(v-if="hasDeleteAnyActionAccess", icon, @click="showDeleteSelectedActionsModal(selected)")
          ...
          tr(@click="props.expanded = !props.expanded")
            ...
            td
              v-layout
                v-flex
                  v-btn(
                    v-if="hasUpdateAnyActionAccess",
                    icon,
                    small,
                    @click.stop="showEditActionModal(props.item)"
                  )
                    v-icon edit
                  v-btn(
                    v-if="hasCreateAnyActionAccess",
                    icon,
                    small,
                    @click.stop="showDuplicateActionModal(props.item)"
                  )
                    v-icon file_copy
                  v-btn.error--text(
                    v-if="hasDeleteAnyActionAccess",
                    icon,
                    small,
                    @click.stop="showDeleteActionModal(props.item._id)"
                  )
                    v-icon(color="error") delete
