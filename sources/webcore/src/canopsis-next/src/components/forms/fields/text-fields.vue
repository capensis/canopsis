      ...
      v-layout.text-field(
        ...
        v-flex(xs12)
          v-text-field(
            ...
            :error-messages="getCollectedErrorMessages(getFieldName(item[itemKey]))",
...
export default {
  ...
  methods: {
    ...
    getCollectedErrorMessages(name) {
      if (this.errors) {
        return this.errors.collect(name).map(error => error.replace(/\[.+]/, ''));
      }
