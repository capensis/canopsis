...
Ember.Application.initializer({
    ...
    initialize: function(container, application) {
         ...
         */
        var mixin = Mixin('contextarraysearch', {
            partials: {
                header: ['search']
            },
            ...
            computeFilterPartForCriterion: function(searchPhrase) {
                ...
                if (searchPhrase.search(/.+={1}".+"/) > -1) {
                    var strs = searchPhrase.split(' ')
                    var ob = strs.map(function(i) {
                        var t = i.split('=');
                        var o = {};
                        o[t[0]] = {
                            '$regex': t[1].substring(1, t[1].length - 1),
                            '$options': 'i'
                        }
                        return o
                    })
                    return JSON.stringify({'$and': ob })
        ...
        application.register('mixin:contextarraysearch', mixin);
    }
});
