...
from bottle import get, put, delete, request, post, response
...
def tree_get():
    ...
    storage = get_storage(
        namespace=namespace, account=account, logging_level=logging.DEBUG)
            ...
            if record.check_write(account=account):
                ...
                for parent in record.parent:
                    ...
                    if parent_rec.check_write(account=account):
                        ...
                        logger.info(
                            ' + No sufficient rights on parent %s' %
                            parent_rec.name)
                ...
                logger.info(
                    ' + No sufficient rights on %s' % parent_rec.crecord_name)
                output[record.name] = {
                    'success': False, 'output': 'No sufficient rights'}
        else:
            output[record.name] = {
                'success': False,
                'output': 'This record have children, remove those child before'}
...
def exportView(_id=None):
    ...
    try:
        ...
        response.headers['Content-Disposition'] = \
            'attachment; filename="%s.json"' % record.name
...
def export_objects():
    ...
    try:
        ...
        response.headers['Content-Disposition'] = \
            'attachment; filename="objects.json"'
        response.headers['Content-Type'] = 'application/json'
...
def add_view(views, storage, account):
    ...
    for view in views:
        ...
        try:
            ...
            logger.info(
                "You don't have right on the parent record: %s" %
                view['parentId'])
            output[view_name] = {
                'success': False,
                'output': "You don't have right on the parent record"}
        ...
        try:
            ...
            output[view['_id']] = {
                'success': False, 'output': "View already exist"}
        except:
        ...
        if record_parent and not record_child:
            if record_parent.check_write(account=account):
                try:
                    ...
                    logger.info(
                        'Error while building view/directory Record : %s' %
                        err)
                    output[view_name] = {
                        'success': False,
                        'output': "Error while building Record: %s" % err}
                    record = None
                ...
                output[view_name] = {
                    'success': False, 'output': "No rights on this record"}
        else:
            logger.error(
                "Parent doesn't exists or view/directory already exists for %s"
                % view_name)
        ...
        if record_child:
            ...
            if parent_id and not parent_id in record_child.parent:
                try:
                    ...
                    record_parent_old = storage.get(
                        record_child.parent[0], account=account)
                    logger.debug(
                        '   + Remove children %s from %s' % (
                            record_child._id, record_parent_old._id))

                    if not record_parent.check_write(account=account) \
                            or not record_parent_old.check_write(
                                account=account):
                        raise Exception('No rights on parent record')

                    record_parent_old.remove_children(record_child)

                    logger.debug('   + Add children %s to %s' % (
                        record_child._id, record_parent._id))
                    record_parent.add_children(record_child)

                    logger.debug('   + Updating all records')

                    storage.put(
                        [record_parent, record_child, record_parent_old],
                        account=account)
                except Exception as err:
                    output[view_name] = {'success': False, 'output': str(err)}
                    logger.error(err)
            ...
            # Rename view
            elif view['crecord_name'] \
                    and record_child.name != view['crecord_name']:
